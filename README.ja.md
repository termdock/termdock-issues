...
*macOS & Windows のための AI ネイティブターミナル — 仕事は Claude Code、Codex、Gemini、OpenCode、GitLab Duo に任せて、GPU アクセラレーションされたワークスペースからすべてを監督。そして背景の 1 ピクセルまで、あなたの自由に。*
...
ソフトウェア開発は変わりました：エージェントが書き、あなたが指揮する。Termdock はその現実のために作られた **macOS と Windows 対応のターミナルファースト開発環境**です。高速な GPU アクセラレーションターミナルの中で、**Claude Code、Codex、Gemini CLI、OpenCode、GitLab Duo** がファーストクラスのセッションとして動き、権限リクエストはスクロールに流されず承認パネルに届き、書かれたコードはコードインテリジェンスとビジュアル Git レビューで検証できます。ワークスペースはローカルに限りません。LAN で別のマシンとペアリングし、あるいは SSH（ベータ）でサーバーへ届けば、リモートターミナルは手元のターミナルの隣に開きます。そして一日中見つめる画面は、完全にあなたのもの：カスタム背景画像、テーマ、フォント、半透明クローム。

### なぜ Termdock？

- **AI エージェントネイティブ** — Claude Code、Codex、Gemini CLI、OpenCode、GitLab Duo をファーストクラスセッションとしてサポート。エージェントはデフォルトでバックグラウンド動作、あなたのペインを奪いません
- **複数マシンをひとつのドックに** — LAN ペア機器や SSH で届くサーバー（ベータ）のワークスペースをマウントし、リモートターミナルをローカル同様に操作
...
- **SSH リモートワークスペース（ベータ）** — SSH で届くサーバーのディレクトリをワークスペース一覧にマウント。ターミナルの操作感はローカルと同じ、短い切断は自動再接続
- **LAN ペアリモート**（v1.18 より） — 同一ネットワークの Termdock 機器を自動発見、一度のペアリングで信頼関係を維持。相手のワークスペースをマウントし、エンドツーエンド暗号化でリモートターミナルを開く。クラウド中継なし
- **Windows ホスト、設定不要** — Windows マシンとのペアリングに手動のファイアウォール設定は不要。インストーラーが処理します
- **ホスト側から丸見え** — 接続中のターミナルセッションをホスト側でリアルタイム閲覧（読み取り専用）
- **スケジュール自動化**（v1.17 より） — cron 形式のプロンプト注入とアイドル keep-alive。既存スケジュールのその場編集に対応
- **ライトテーマとサーフェスの刷新** — ライト / ダークで一貫した配色、ダイアログはソリッドかガラスかを設定で選択可能
- **アプリ内バグ報告** — 新しい Report メニューからそのまま報告を送信
...
流れていくスクロールをずっと見張っている必要は、もうありません。Termdock では **Claude Code、Codex、Gemini CLI、OpenCode、GitLab Duo** が管理されたエージェントセッションとして動きます — デフォルトでバックグラウンド動作なので、エージェントが黙々と働いている間も、あなたのペインはあなたのものです。
...
プロジェクトが 1 台のマシンに収まっているとは限りません。Termdock はリモートワークスペースをローカルの隣にマウントします — 同じグリッド、同じタブ、同じショートカット。

- **LAN ペアリモート** — 同一ネットワークの機器を自動発見。一度ペアリングすれば信頼関係を維持し、共有ワークスペースをエンドツーエンド暗号化チャネルでマウント。クラウド中継なし
- **SSH リモートワークスペース（ベータ）** — アドレスを入力してサインインすれば、SSH で届くどのサーバーでも作業可能。現在はターミナル中心で、ファイル / Git パネルのリモート対応も順次追加
- **切れても自動復帰** — 短い切断やホストの再起動は自動再接続。中断されたセッションは復帰猶予の間保持されます
- **ホストが主導権を持つ** — 共有は設定で有効にするまでオフ。危険なルートは拒否され、接続中のセッションは可視化され個別に終了できます
...
- **SSH リモートの仕上げ** — ベータから安定版へ
- **リモートのファイル / Git パネル** — マウントしたリモートワークスペースでターミナルだけでなくファイル閲覧と Git レビューも

### その先

- **ブラウザアクセス** — デスクトップアプリの外からターミナルへ
